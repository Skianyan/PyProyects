...
def GenerateSphere(divs, color):
    ...
    for d in range(0, divs):
        for i in range(0, divs):
            ...
            tri0 = [i0, i1, i2]
            tri1 = [i0, i0 + divs, i1]
            trianglesS.append(gf.Triangle(tri0, color))
            trianglesS.append(gf.Triangle(tri1, color))

    return gf.Model(verticesS, trianglesS, gf.Vertex(0, 0, 0), 1.0)
...
instances = [gf.Instance(sphere, gf.Vertex(-1, 0, 25), gf.MakeOYRotationMatrix(200), 5),
             gf.Instance(dodecahedron, gf.Vertex(-10, 5, 30), gf.MakeOYRotationMatrix(200), 0.5)]

# Parámetros de la cámara
camera = gf.Camera(gf.Vertex(-3, 1, 2), gf.MakeOYRotationMatrix(0))
...
lights = [gf.Light(LT_AMBIENT, 0.2),
          gf.Light(LT_DIRECTIONAL, 0.2, gf.Vertex(1, 0, 1)),
          gf.Light(LT_POINT, 0.6, gf.Vertex(-7, 0, 5))]
